...
if ( !function_exists( 'studeon_get_template_page_link' ) ) {
	//Handler of the add_filter('post_type_archive_link', 'studeon_get_template_page_link', 10, 2 );
...
if ( !function_exists( 'studeon_wp_title' ) ) {
	// Handler of the add_filter( 'wp_title', 'studeon_wp_title');
	// Handler of the add_filter( 'wp_title_parts', 'studeon_wp_title');
	// Handler of the add_filter( 'document_title_parts', 'studeon_wp_title');
	...
	function studeon_get_post_terms($delimiter=', ', $id=false, $taxonomy='category') {
		...
		if ( !empty( $terms ) ) {
			foreach( $terms as $term )
				$output .= ($output ? $delimiter : '') . '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '" title="' . sprintf( esc_attr__( 'View all posts in %s', 'studeon' ), $term->name ) . '">' . esc_html( $term->name ) . '</a>';
		}
		return $output;
	}
}
...
if ( ! function_exists('studeon_get_privacy_text' ) ) {
    function studeon_get_privacy_text() {
        ...
        return apply_filters( 'studeon_filter_privacy_text', wp_kses_post(
                $privacy_text
