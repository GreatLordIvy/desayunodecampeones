...
?><div id="post-<?php the_ID(); ?>"	<?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) ); ?>><?php
	...
	set_query_var('trx_addons_args_featured', apply_filters('trx_addons_filter_args_featured', array(
		...
		'hover' => 'zoomin',
		'thumb_size' => trx_addons_get_thumb_size($args['columns'] > 2 ? 'big-blogger' : 'big-blogger')
	), 'team-classic'));
	...
	?><div class="sc_blogger_item_content entry-content"><?php
		...
		if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) {
			?><div class="sc_blogger_item_excerpt">
				...
				// More button
				if ( $show_more ) {
					?><div class="sc_blogger_item_button sc_item_button"><a href="<?php echo esc_url($post_link); ?>" class="sc_button sc_button_simple"><?php esc_html_e('Read more', 'studeon'); ?></a></div><?php
				}
