	...
	function studeon_customizer_css_template() {
		...
		if (count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<' . esc_html($tmpl_holder) . ' type="text/html" id="tmpl-studeon-color-scheme-'.esc_attr($scheme).'">'
					...
					. '<div class="studeon_override_options_field_preview">'
						. (studeon_is_inherit($field['val']) 
								...
								: ($field['val'] && $field['type']=='image' 
										? '<img src="' . esc_url($field['val']) . '" alt="' . esc_html(basename($field['val'])) . '">'
										: basename($field['val'])
									)
							)
					. '</div>';
...
if (!function_exists('studeon_options_override_save_options')) {
	//Handler of the add_action('save_post', 'studeon_options_override_save_options');
	function studeon_options_override_save_options($post_id) {
		...
		foreach ($options as $k=>$v) {
			...
			$meta[$k] = isset($_POST['studeon_override_options_field_' . $k])
							? $_POST['studeon_override_options_field_' . $k]
							: ($v['type']=='checkbox' ? 0 : '');
		}
